...
Desde o ONTAP 9.12.1, os snapshots não são apenas somente leitura, mas também podem ser protegidos contra exclusão acidental ou intencional. Esse recurso é chamado de tamperproof snapshots. Um período de retenção pode ser definido e aplicado por meio de uma política de snapshot. Os snapshots resultantes não podem ser excluídos até atingirem a data de expiração. Não há substituições administrativas ou do centro de suporte.
