...
* Use uma única interface lógica (LIF) para cada SVM em cada nó no cluster ONTAP. Recomendações anteriores de uma LIF por datastore não são mais necessárias. Embora o acesso direto (LIF e datastore no mesmo nó) seja o ideal, não se preocupe com o acesso indireto, pois o efeito no desempenho é geralmente mínimo (microssegundos).
...
[TIP]
====
* O valor máximo para nconnect em vSphere 8 é 4, e o valor padrão é 1. O limite máximo de valor em vSphere pode ser aumentado para cada host individualmente por meio de configurações avançadas, embora geralmente não seja necessário.
* Um valor de 4 é recomendado para ambientes que exigem mais desempenho do que uma única conexão TCP pode fornecer.
* Tenha em mente que o ESXi tem um limite de 256 conexões NFS, e cada conexão nconnect conta para esse total. Por exemplo, dois datastores com nconnect=4 contariam como oito conexões no total.
* É importante testar o impacto do nconnect no desempenho do seu ambiente antes de implementar alterações em larga escala em ambientes de produção.
...
* Conforme mencionado em link:vmware-vsphere-settings.html["definições"], se você não estiver usando o vSphere CSI para Kubernetes, deverá definir o newSyncInterval por https://knowledge.broadcom.com/external/article/386364/reducing-excessive-vsan-cnssync-warnings.html["VMware KB 386364"^]
...
* Embora o ONTAP ofereça uma estrutura de namespace de volume flexível para organizar volumes em uma árvore usando junções, essa abordagem não tem valor para vSphere. Ele cria um diretório para cada máquina virtual na raiz do datastore, independentemente da hierarquia de namespace do sistema de storage. Assim, a melhor prática é simplesmente montar o caminho de junção dos volumes para vSphere no volume raiz do SVM, que é como o ONTAP tools for VMware vSphere provisiona datastores. Não ter caminhos de junção aninhados também significa que nenhum volume depende de outro volume além do volume raiz, e que colocar um volume offline ou destruí-lo, mesmo intencionalmente, não afeta o caminho para outros volumes.
