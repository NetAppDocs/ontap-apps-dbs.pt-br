...
vSphere inclui suporte incorporado para múltiplos caminhos para dispositivos de storage. vSphere pode detectar o tipo de dispositivo de storage para sistemas de storage suportados e configura automaticamente a pilha de multipath para suportar as capacidades do sistema de storage em uso, independentemente do protocolo utilizado ou se estiver usando ASA, AFF, FAS ou ONTAP definido por software.
...
NOTE: Consulte a link:https://configmax.broadcom.com/guest?vmwareproduct=vSphere&release=vSphere%208.0&categories=2-0["Ferramenta VMware Configuration Maximums"^] para obter os limites suportados mais recentes no ESXi.
...
* Para redes iSCSI ou NVMe/TCP, use múltiplas interfaces de rede VMkernel em diferentes sub-redes não roteadas com agrupamento de NICs quando múltiplos switches virtuais estiverem presentes. Você também pode usar múltiplas NICs físicas conectadas a múltiplos switches físicos para fornecer HA e aumentar a taxa de transferência. No ONTAP, configure um grupo de interfaces em modo único para failover com dois ou mais links conectados a dois ou mais switches, ou use LACP ou outra tecnologia de agregação de links com grupos de interfaces multimodo para fornecer HA e os benefícios da agregação de links. Lembre-se de que o NVMe/TCP deve ser habilitado por porta VMkernel e por interface física, e você deve alinhá-los de forma que as portas VMkernel NVMe/TCP tenham acesso às NICs físicas com NVMe/TCP habilitado. A figura a seguir ilustra a conectividade multipath com protocolos SAN baseados em IP. image:vsphere_ontap_image2.png["conectividade multipath"]
