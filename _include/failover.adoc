...
Durante operações de manutenção de rotina, uma operação de takeover de armazenamento ou de switchover MetroCluster deve ser transparente, exceto por uma possível breve pausa nas operações enquanto os caminhos de rede são alterados. No entanto, o networking pode ser complicado e é fácil cometer erros, por isso a NetApp recomenda fortemente testar minuciosamente as operações de takeover e switchover antes de colocar um sistema de storage em produção. Fazer isso é a única maneira de garantir que todos os caminhos de rede estejam configurados corretamente. Em um ambiente SAN, verifique cuidadosamente a saída do comando  `sanlun lun show -p` para garantir que todos os caminhos primários e secundários esperados estejam disponíveis.
