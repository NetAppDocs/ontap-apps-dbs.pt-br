...
Os diagramas mostram o que acontece durante as operações normais, de failover e de failback. Para facilitar a ilustração, eles representam um LUN replicado. Em uma configuração real de SnapMirror active sync, a replicação é baseada em volumes AFF, onde cada volume contém um ou mais LUNs, mas para simplificar a visualização, a camada de volume foi removida.
