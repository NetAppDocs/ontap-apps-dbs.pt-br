...
Como exemplo de referência, um sistema HA AFF A800 de dois nós mais antigo é capaz de atender até um milhão de IOPS aleatórios antes que a latência ultrapasse um milissegundo. Espera-se que pouquíssimas cargas de trabalho individuais atinjam tais níveis. A utilização plena desse array de sistema AFF A800 envolverá a hospedagem de múltiplas cargas de trabalho e, para fazer isso com segurança enquanto se garante a previsibilidade, são necessários controles de QoS.

Existem dois tipos de qualidade do serviço (QoS) no ONTAP: IOPS e largura de banda. Os controles de QoS podem ser aplicados a AFF SVMs, AFF volumes, AFF arquivos e AFF/ASA LUNs e namespaces.
