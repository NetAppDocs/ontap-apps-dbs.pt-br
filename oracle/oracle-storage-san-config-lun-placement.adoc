...
[role="lead"]
O posicionamento ideal dos LUNs de banco de dados dentro dos volumes ONTAP em sistemas AFF e FAS depende principalmente de como os diversos recursos do ONTAP serão utilizados.

[]
====
* Esta seção aplica-se aos sistemas AFF e FAS.
* ASA é uma plataforma SAN simplificada e não expõe uma camada de abstração de volume,
