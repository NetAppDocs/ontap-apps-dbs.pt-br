...
summary: Alinhamento de LUN com bancos de dados Oracle no ASA r2 
searchtitle: Alinhamento de LUN para E/S de banco de dados Oracle com ASA r2 
---
= Alinhamento LUN
:hardbreaks:
:allow-uri-read: 
...
Verifique o alinhamento usando utilitários como `sio` ou `dd` que pode gerar E/S em um tamanho de bloco definido. As estatísticas de alinhamento de E/S no sistema de armazenamento podem ser visualizadas com o `stats` comando. Ver link:../notes/wafl_alignment_verification.html["Verificação do alinhamento do WAFL"] Para obter mais informações.
