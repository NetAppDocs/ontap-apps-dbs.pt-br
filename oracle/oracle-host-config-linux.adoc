...
Uma opção de montagem adicional, chamada  `nosharecache`, é necessária quando (a) o DNFS está habilitado e (b) um compartilhamento NFS de origem é montado mais de uma vez em um único servidor (c) com uma montagem NFS aninhada. Essa configuração é vista principalmente em ambientes que suportam aplicativos SAP. Por exemplo, um único compartilhamento NFS em um NetApp system pode ter um diretório localizado em  `/vol/oracle/base` e um segundo em  `/vol/oracle/home`. Se  `/vol/oracle/base` for montado em  `/oracle` e  `/vol/oracle/home` for montado em  `/oracle/home`, o resultado serão montagens NFS aninhadas que se originam na mesma origem.

O sistema operacional pode detectar que `/oracle` e `/oracle/home` residem no mesmo compartilhamento NFS, que é o mesmo sistema de arquivos de origem. O sistema operacional então usa o mesmo identificador de dispositivo para acessar os dados. Fazer isso melhora o uso do armazenamento em cache do sistema operacional e certas outras operações, mas interfere no DNFS. Se o DNFS precisar acessar um arquivo, como o `spfile`, em `/oracle/home`, ele pode tentar erroneamente usar o caminho errado para os dados. O resultado é uma operação de E/S com falha. Nessas configurações, adicione a opção de montagem `nosharecache` a qualquer sistema de arquivos NFS que compartilhe um compartilhamento NFS de origem com outro sistema de arquivos NFS nesse host. Fazer isso força o sistema operacional Linux a alocar um identificador de dispositivo independente para esse sistema de arquivos.
