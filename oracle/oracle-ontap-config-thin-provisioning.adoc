...
Por exemplo, considere um banco de dados em um único LUN de 250GB em um volume de 1TB em um sistema AFF. A criação de um snapshot resultaria imediatamente na reserva de 250GB adicionais de espaço no volume para garantir que o volume não fique sem espaço por nenhum motivo. O uso de reservas fracionárias geralmente é um desperdício, pois é extremamente improvável que todos os bytes no volume do banco de dados precisem ser sobrescritos. Não há motivo para reservar espaço para um evento que nunca ocorre. Ainda assim, se um cliente não puder monitorar o consumo de espaço em um sistema de storage e precisar ter certeza de que o espaço nunca se esgotará, reservas fracionárias de 100% seriam necessárias para usar snapshots.


TIP: A abstração de volume não está presente no ASA, e, portanto, essa plataforma não usa reservas fracionárias.
...
Existe sempre a possibilidade de a compressibilidade de um conjunto de dados se alterar, o que resultaria em aumento do consumo de espaço real. Esse aumento de consumo significa que a compactação deve ser gerenciada, assim como outras formas de thin provisioning, em termos de monitoramento e, em um sistema AFF, usando  `volume-autogrow` e  `snapshot-autodelete`.
...
[TIP]
====
*NetApp recomenda* as seguintes configurações de reserva com as plataformas AFF e FAS:
...
A eficiência do provisionamento dinâmico de LUNs ativos em um ambiente de sistema de arquivos pode ser perdida ao longo do tempo à medida que os dados são excluídos. A menos que os dados excluídos sejam sobrescritos com zeros (veja também link:oracle-storage-san-config-asm ru.html[ASMRU] ou o espaço seja liberado com a recuperação de espaço TRIM/UNMAP, os dados "apagados" ocupam cada vez mais espaço em branco não alocado no sistema de arquivos. Além disso, o provisionamento dinâmico de LUNs ativos tem utilidade limitada em muitos ambientes de banco de dados, porque os arquivos de dados são inicializados com seu tamanho total no momento da criação.
