...
Para proteger os dados dessa situação, ONTAP permite que volumes, LUNs ou namespaces NVMe sejam configurados para proteção especial contra falhas de NVRAM. Quando acionado, esse mecanismo de proteção faz com que recursos de storage entrem em um estado chamado NVFAIL. Esse estado resulta em erros de E/S que causam o encerramento do aplicativo para que ele não utilize dados obsoletos. Os dados não devem ser perdidos, pois qualquer gravação confirmada deve estar presente no array de storage.

Os próximos passos habituais são o desligamento completo dos hosts por um administrador, antes de colocar manualmente os LUNs, namespaces ou volumes NFS on-line novamente. Embora esses passos possam envolver algum trabalho, essa abordagem é a maneira mais segura de garantir a integridade de dados. Nem todos os dados exigem essa proteção, por isso o comportamento do NVFAIL pode ser configurado em um nível granular.
