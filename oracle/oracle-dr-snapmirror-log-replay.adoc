...
O requisito mais importante é o isolamento dos arquivos de dados em um ou mais contêineres dedicados. Isso pode ser feito de várias maneiras, incluindo colocar todos os arquivos de dados em um único LUN/namespace, dentro de um único volume NFS AFF ou colocá-los em um grupo de consistência ONTAP multi-volume ou multi-LUN/namespace. O ponto crucial é que os arquivos de dados não devem ser contaminados por nenhum outro tipo de arquivo. O motivo é garantir que a replicação dos arquivos de dados seja totalmente independente da replicação de outros tipos de dados, como archive logs. Para obter informações adicionais sobre layouts de arquivos e detalhes importantes sobre como garantir que o layout de storage seja compatível com snapshot, consulte link:oracle-dp-online-backup.html#data-layout["Layout dos dados"].
